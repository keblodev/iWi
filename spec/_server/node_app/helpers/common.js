...
var ORM         = require(process.env.SERVER_APP_PATH + '/node_modules/orm');


common.ORM = ORM;

common.protocol = function () {
  return require(process.env.SERVER_APP_PATH + "/configs/db").protocol;
};
...
common.hasConfig = function (proto) {
  ...
  try {
    config = require(process.env.SERVER_APP_PATH + "/configs/db");
  } catch (ex) {
    return 'not-found';
  }
...
common.getConfig = function () {
    var config = require(process.env.SERVER_APP_PATH + "/configs/db");
